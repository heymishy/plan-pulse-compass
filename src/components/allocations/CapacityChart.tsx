    ...
    return iterations.map((iteration, index) => {
      ...
      teams.forEach(team => {
        ...
        if (capacity.allocatedPercentage <= 100) {
          totalAllocated += capacity.capacityHours * (capacity.allocatedPercentage / 100);
        } else {
          totalAllocated += capacity.capacityHours;
          overAllocated += capacity.capacityHours * ((capacity.allocatedPercentage - 100) / 100);
        }
      });
          ...
          <div style={{ width: '100%', height: '400px' }}>
            <ResponsiveContainer>
              <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <Bar dataKey="allocated" stackId="a" fill="#10b981" name="Allocated" />
                <Bar dataKey="overAllocated" stackId="a" fill="#ef4444" name="Over Allocated" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
            ...
            {teams.map(team => {
              const teamData = iterations.map((iteration, index) => {
                ...
                return {
                  ...
                  hours: Math.round(capacity.capacityHours * (capacity.allocatedPercentage / 100)),
                  capacity: Math.round(capacity.capacityHours),
                };
              });

              const avgUtilization = teamData.reduce((sum, d) => sum + d.percentage, 0) / teamData.length;
              ...
              return (
                <div key={team.id} className="p-4 border rounded-lg">
                  ...
                  <div className="space-y-2">
                    {teamData.map(data => (
                      <div key={data.iteration} className="flex items-center justify-between text-sm">
                        <span>Iter {data.iteration}:</span>
                        <div className="flex items-center space-x-2">
                          <div className={`w-3 h-3 rounded-full ${
                            data.percentage === 100 ? 'bg-green-500' :
                            data.percentage > 100 ? 'bg-red-500' :
                            data.percentage > 0 ? 'bg-yellow-500' : 'bg-gray-300'
                          }`}></div>
