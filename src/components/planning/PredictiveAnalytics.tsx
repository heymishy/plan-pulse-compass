...
const PredictiveAnalytics = () => {
  ...
  const predictiveData = useMemo(() => {
    ...
    for (let i = 0; i < periods; i++) {
      ...
      const capacityTrend =
        85 + Math.sin(i * 0.5) * 5 + (Math.random() - 0.5) * 3;
      const deliveryTrend =
        78 + Math.cos(i * 0.3) * 8 + (Math.random() - 0.5) * 4;
    ...
    () => [
      {
        ...
        insight:
          'Team utilization is trending upward by 3% per quarter. Current trajectory suggests potential overutilization by Q3.',
        confidence: 84,
      ...
      },
      {
        ...
        insight:
          'Based on historical patterns, delivery efficiency typically drops 8% during Q4 due to holidays and year-end activities.',
        confidence: 91,
      ...
      },
      {
        ...
        insight:
          'Cross-training 3 team members in cloud architecture could increase project flexibility by 25%.',
        confidence: 76,
  ...
  const getChartData = () => {
    switch (analysisType) {
      case 'capacity':
        return predictiveData.map(d => ({
          ...d,
          value: d.capacityUtilization,
        }));
      case 'delivery':
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <Select
            value={analysisType}
            onValueChange={(value: 'capacity' | 'delivery' | 'budget') =>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {predictiveData.length > 0
                ? predictiveData[predictiveData.length - 1].confidence
                : 0}
              %
            </div>
                  ...
                  <tbody>
                    {predictiveData.map((month, index) => (
                      <tr key={index} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-2">
                          {month.capacityUtilization.toFixed(1)}%
                        </td>
                        <td className="p-2">
                          {month.deliveryEfficiency.toFixed(1)}%
                        </td>
                        <td className="p-2">
                          {month.budgetUtilization.toFixed(1)}%
                        </td>
            ...
            <CardContent>
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 mb-4">
                  Model different business scenarios and their predicted impact
                  on team performance.
                </p>
                <Button variant="outline">Configure Scenarios</Button>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
