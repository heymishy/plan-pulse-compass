...
import { 
  Search, TrendingUp, AlertTriangle, Users, Target, 
...
const ScenarioAnalysisDashboard = () => {
  ...
  const getSkillCoverageData = () => {
    ...
    const skillCoverage = analysisResults.teamMatches.reduce((acc: any, team: any) => {
    ...
    return Object.entries(skillCoverage).map(([skill, data]: [string, any]) => ({
      skill,
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Scenario Analysis Dashboard</h1>
          <p className="text-gray-600">Advanced project-team matching and analytics</p>
        </div>
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-green-600">
                    {Math.round(analysisResults.teamMatches[0]?.overallScore || 0)}%
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-green-600">
                    {analysisResults.teamMatches.filter((t: any) => t.overallScore >= 70).length}
                  </div>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={getTeamEfficiencyData()}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="efficiency" fill="#8884d8" name="Overall Score" />
                    <Bar dataKey="skillMatch" fill="#82ca9d" name="Skill Match" />
                    <Bar dataKey="availability" fill="#ffc658" name="Availability" />
                  </BarChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {analysisResults.teamMatches.slice(0, 2).map((team: any) => (
                <Card key={team.teamId}>
                  ...
                  <CardContent>
                    <ResponsiveContainer width="100%" height={300}>
                      <RadarChart data={team.skillBreakdown.map((skill: any) => ({
