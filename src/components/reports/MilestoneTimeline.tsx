        ...
        <div key={milestone.id} className="flex items-start">
          <div className="flex flex-col items-center mr-4 pt-1">
            ...
            {index < sortedMilestones.length - 1 && (
              <div className="w-px h-16 bg-gray-300 mt-1" />
            )}
          </div>
          <div>
            ...
            <p
              className={`text-sm font-medium capitalize ${getStatusColor(milestone.status)}`}
            >
              Status:{' '}
              {milestone.status
            ...
            {milestone.status === 'completed' &&
              milestone.actualCompletionDate && (
                <p className="text-sm text-green-600">
                  Completed:{' '}
                  {format(new Date(milestone.actualCompletionDate), 'PPP')}
                </p>
              )}
          </div>
        </div>
