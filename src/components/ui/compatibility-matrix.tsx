...
export interface CompatibilityMatrixProps<
  R extends MatrixItem,
  C extends MatrixItem,
> {
  // Data
  rowItems: R[];
  columnItems: C[];
...
>({
    ...
    if (sortable) {
      filtered.sort((a, b) => {
        if (sortBy === 'name') {
          ...
          const aScore =
            columnItems.reduce((sum, col) => {
              const cellData = matrixData.find(
                d => d.rowItem.id === a.id && d.columnItem.id === col.id
              );
          ...
          const bScore =
            columnItems.reduce((sum, col) => {
              const cellData = matrixData.find(
                d => d.rowItem.id === b.id && d.columnItem.id === col.id
              );
              return sum + (cellData?.score.score || 0);
            }, 0) / columnItems.length;
    ...
    (e: React.KeyboardEvent, rowIndex: number, colIndex: number) => {
      switch (e.key) {
        case 'ArrowRight':
          ...
          setFocusedCell([
            rowIndex,
            Math.min(colIndex + 1, columnItems.length - 1),
          ]);
          break;
        case 'ArrowLeft':
          e.preventDefault();
        ...
        case 'ArrowDown':
          e.preventDefault();
          setFocusedCell([
            Math.min(rowIndex + 1, filteredRows.length - 1),
            colIndex,
          ]);
          break;
        case 'ArrowUp':
  ...
  const handleExport = useCallback(
    (format: ExportFormat) => {
      ...
      const exportData = matrixData.filter(
        data =>
      ...
      <Card className={cn('flex items-center justify-center p-8', className)}>
        <div className="text-center">
          ...
          <p className="text-lg font-medium text-red-600">
            Error calculating compatibility
          </p>
          <p className="text-sm text-muted-foreground">
            ...
            <div className="flex items-center gap-4 mt-4">
              ...
              {filterable && (
                <div className="flex items-center gap-2">
                  ...
                  <label
                    htmlFor="compatibility-threshold"
            ...
            <tbody>
              {filteredRows.map((rowItem, rowIndex) => (
                <tr key={rowItem.id}>
                  ...
                  >
                    {renderRowHeader ? (
                      renderRowHeader(rowItem, rowIndex)
                    ) : (
                      <div className="space-y-1">
                        <div
                  ...
                  {columnItems.map((columnItem, colIndex) => {
                    const cellData = matrixData.find(
                      d =>
                        d.rowItem.id === rowItem.id &&
                        d.columnItem.id === columnItem.id
                    );
