...
export function PerformanceMetricsWidget({
  ...
  const [favoriteMetrics, setFavoriteMetrics] = useState<Set<string>>(
    new Set()
  );
  const [comparisonMode, setComparisonMode] = useState(false);
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  const [showTable, setShowTable] = useState(false);
  const [editingThresholds, setEditingThresholds] = useState<string | null>(
    null
  );
  ...
  const filteredMetrics = useMemo(() => {
    ...
    return data.metrics.filter(metric => {
      ...
      if (
        ...
        !selectedStatuses.includes(metric.status)
      ) {
        return false;
      }
  ...
  const handleThresholdEdit = useCallback(
    (metricId: string, metric: PerformanceMetric) => {
        ...
        <div className="mb-4">
          ...
          >
            ...
            <Button
              ...
              data-testid={
                isRefreshPaused ? 'play-refresh-button' : 'pause-refresh-button'
              }
            >
              {isRefreshPaused ? (
        ...
        <CardContent>
            ...
            <div className="mb-6" data-testid="alerts-section">
              ...
              <div className="space-y-3">
                {['critical', 'warning', 'info'].map(severity => {
                  const severityAlerts = data.alerts.filter(
          ...
          {groupByCategory ? (
                      ...
                      {categoryMetrics.map(metric => {
                        ...
                        return (
                          <Card
                            ...
                            onDoubleClick={() =>
                              drillDownEnabled && onDrillDown?.(metric)
                            }
                          >
                                ...
                                >
                                  {lazyLoadDetails ? (
                                    <div
                                      ...
                                      data-testid="metric-details-loading"
                                    >
                                    ...
                                    <>
                                      ...
                                      {showTrendCharts && (
                                        <div data-testid="trend-chart">
                                          ...
                                          <div
                                            className="h-32 bg-gray-100 rounded flex items-center justify-center text-sm text-muted-foreground"
                                            data-testid="historical-data"
                                          >
                                            Historical data visualization for{' '}
                                            {metric.name}
                                          </div>
                                        </div>
                                      )}
            ...
            >
              {customTemplate
                ? filteredMetrics.map(metric => (
                    <div key={metric.id}>{customTemplate(metric)}</div>
                  ))
                : filteredMetrics.map(metric => {
                    const StatusIcon = getStatusIcon(metric.status);
                    const TrendIcon = getTrendIcon(metric.trend);
                          ...
                          <div className="mb-3">
                            ...
                            >
                              ...
                              <div
                                ...
                                style={{
                                  width: `${(metric.threshold.good / metric.threshold.critical) * 100}%`,
                                }}
                                data-testid="threshold-good"
                              />
                              <div
                              ...
                              <div
                                ...
                                style={{
                                  left: `${Math.min(100, (metric.value / metric.threshold.critical) * 100)}%`,
                                }}
                              />
                            </div>
                          </div>
                                ...
                                <>
                                  ...
                                  {showTrendCharts && metric.historical && (
                                    <div data-testid="trend-chart">
                                      ...
                                      <div
                                        className="h-32 bg-gray-100 rounded p-2"
                                        data-testid="historical-data"
                                      >
                                        <div className="flex items-end justify-between h-full">
                                          {metric.historical.map(
                                            (point, index) => (
                                              <div
                                                key={index}
                                        ...
                                        <div className="space-y-3 p-3 border rounded-lg">
                                          ...
                                          {!validateThresholds(
                                            ...
                                            <p className="text-xs text-red-600">
                                              Warning threshold must be less
  ...
  // Helper function to render metrics (for grouped display)
  const renderMetrics = (metrics: PerformanceMetric[]) => {
