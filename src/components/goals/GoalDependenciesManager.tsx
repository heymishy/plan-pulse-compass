...
const GoalDependenciesManager: React.FC<GoalDependenciesManagerProps> = ({ goalId }) => {
  ...
  const availableDependencies = goals.filter(g => 
    g.id !== selectedGoal && 
    !currentGoal?.dependencies.includes(g.id) &&
    !wouldCreateCircularDependency(selectedGoal, g.id)
  );

  function wouldCreateCircularDependency(goalId: string, dependencyId: string): boolean {
  ...
  function checkDependencyChain(goal: Goal, targetId: string, visited: Set<string>): boolean {
  ...
  const addDependency = () => {
    ...
    const updatedDependencies = [...currentGoal.dependencies, newDependency];
  ...
  const removeDependency = (dependencyId: string) => {
    ...
    const updatedDependencies = currentGoal.dependencies.filter(id => id !== dependencyId);
    updateGoal(currentGoal.id, { dependencies: updatedDependencies });
  };
      ...
      {currentGoal ? (
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <h4 className="font-medium mb-2">Dependencies ({currentGoal.dependencies.length})</h4>
                      ...
                      return (
                        <div key={depId} className="flex items-center justify-between p-2 border rounded">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm font-medium">{depGoal.title}</span>
                          </div>
                          <Button
                            variant="ghost"
                            size="sm"
                  ...
                  return dependentGoals.length > 0 ? (
                    <div className="space-y-2">
                      {dependentGoals.map((goal) => (
                        <div key={goal.id} className="flex items-center space-x-2 p-2 border rounded bg-blue-50">
              ...
              {currentGoal.dependencies.some(depId => 
                ...
                <div className="flex items-center space-x-2 p-3 bg-yellow-50 border border-yellow-200 rounded">
                  ...
                  <span className="text-sm text-yellow-800">
                    Potential circular dependency detected. Review your dependency chain.
                  </span>
                </div>
              )}
            </CardContent>
        ...
        <Card>
          <CardContent className="text-center py-8 text-gray-500">
            {goals.length === 0 ? 'No goals available' : 'Select a goal to manage its dependencies'}
          </CardContent>
        </Card>
      )}
    </div>
