...
const InteractiveGoalNode: React.FC<InteractiveGoalNodeProps> = ({
  data,
  selected,
}) => {
  ...
  return (
    <div
      className={`
        ...
        transition-all duration-200 hover:shadow-lg ${getStatusColor(
          data.status
        )}
    ...
    >
      ...
      <Handle
        ...
        className="opacity-0 hover:opacity-100 bg-blue-500"
      />
      <Handle
        type="source"
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center space-x-1">
          ...
          {data.ownerId && <Users className="h-3 w-3 text-gray-500" />}
        </div>
      </div>
      ...
      <div
        className={`font-medium mb-3 line-clamp-2 ${
          data.isNorthStar ? "text-lg" : "text-sm"
        }`}
      >
        {data.title}
      </div>
      ...
      {data.metric && data.metric.target > 0 && (
        <div className="mb-3 relative">
          ...
          <div className="text-xs text-gray-500 mt-1 font-mono">
            {data.metric.current.toLocaleString()} /{" "}
            {data.metric.target.toLocaleString()} {data.metric.unit}
          </div>
        </div>
      )}
