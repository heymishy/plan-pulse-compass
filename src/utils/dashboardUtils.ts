...
import { Cycle, Allocation, ActualAllocation, IterationReview, Project, Epic } from '@/types';
 ...
 */
export const findCurrentCycle = (cycles: Cycle[], type: 'quarterly'): Cycle | undefined => {
    const today = startOfToday();
    return cycles
        .filter(c => c.type === type && c.status === 'active')
        .find(c => isWithinInterval(today, { start: parseISO(c.startDate), end: parseISO(c.endDate) }));
};
 ...
 */
export const getCurrentIteration = (cycles: Cycle[], currentQuarter?: Cycle): Cycle | undefined => {
    const today = startOfToday();
    // Prefer iterations from the current quarter if available
    const sourceCycles = currentQuarter 
        ? cycles.filter(c => c.type === 'iteration' && c.parentCycleId === currentQuarter.id) 
        : cycles.filter(c => c.type === 'iteration');

    const iterations = sourceCycles.sort((a, b) => parseISO(b.startDate).getTime() - parseISO(a.startDate).getTime());
    
    // Find an iteration that is currently active.
    const activeIteration = iterations.find(c => isWithinInterval(today, { start: parseISO(c.startDate), end: parseISO(c.endDate) }));
    if(activeIteration) return activeIteration;

    // Fallback: find the most recent one that has already started.
    const mostRecentStarted = iterations.find(c => isBefore(parseISO(c.startDate), today) || parseISO(c.startDate).getTime() === today.getTime());
...
export const getDashboardData = (
    ...
    const iterationNumber = currentIteration ? getIterationNumberFromName(currentIteration.name) : null;
    
    if (currentQuarter && iterationNumber !== null) {
        const planned = allocations
            .filter(a => a.cycleId === currentQuarter.id && a.iterationNumber === iterationNumber)
    ...
    if (currentQuarter) {
        const quarterIterations = cycles.filter(c => c.parentCycleId === currentQuarter.id && c.type === 'iteration');
        const quarterIterationNumbers = quarterIterations.map(i => getIterationNumberFromName(i.name)).filter(n => n !== null) as number[];

        // Epics progress
        const epicsInQuarter = new Set(
            allocations
                .filter(a => a.cycleId === currentQuarter.id && quarterIterationNumbers.includes(a.iterationNumber) && a.epicId)
                .map(a => a.epicId!)
        );
        const completedEpicsInQuarter = new Set(
            iterationReviews
                .filter(r => r.cycleId === currentQuarter.id && quarterIterationNumbers.includes(r.iterationNumber))
                .flatMap(r => r.completedEpics)
        );

        // Milestones progress
        const milestonesInQuarter = projects.flatMap(p => p.milestones).filter(m => 
            isWithinInterval(parseISO(m.dueDate), { start: parseISO(currentQuarter.startDate), end: parseISO(currentQuarter.endDate) })
        );
        ...
        quarterlyProgress = {
            ...
            milestones: {
                ...
                percentage: milestoneTotal > 0 ? (milestoneCompleted / milestoneTotal) * 100 : 0
            }
        };
    }
