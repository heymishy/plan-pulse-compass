...
import { 
  Project, Team, Person, Skill, PersonSkill, ProjectSkill, ProjectSolution, Solution,
  Allocation, Cycle, Division, Role 
} from '@/types';
import { 
  ProjectFeasibilityAnalysis, TeamRecommendation, BudgetImpactAnalysis,
  PlanningScenario, ProjectSkillRequirement, FeasibilityRisk, DivisionBudget
} from '@/types/planningTypes';
import { calculatePersonCost, calculateProjectCost } from '@/utils/financialCalculations';
...
export const analyzeProjectsFeasibility = (
  ...
  projectIds.forEach(projectId => {
    ...
    const budgetImpact = calculateProjectBudgetImpact(project, teamRecommendations, data);
    
    // Assess risks
    const riskFactors = assessProjectRisks(project, skillRequirements, teamRecommendations, data);
...
const getProjectSkillRequirements = (
  ...
  const projectSolutions = data.projectSolutions.filter(ps => ps.projectId === projectId);
  ...
  const directSkills = data.projectSkills.filter(ps => ps.projectId === projectId);
...
const calculateTeamCostEfficiency = (teamId: string, data: PlanningEngineData): number => {
  const team = data.teams.find(t => t.id === teamId);
  if (!team) return 0;
  
  const teamMembers = data.people.filter(p => p.teamId === teamId && p.isActive);
...
const getTeamAvailabilityWindows = (teamId: string, data: PlanningEngineData): any[] => {
  const now = new Date();
  const futureQuarters = data.cycles.filter(c => 
    c.type === 'quarterly' && new Date(c.endDate) > now
  ).sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime());
  
  return futureQuarters.map(quarter => {
    const quarterAllocations = data.allocations.filter(a => 
      a.teamId === teamId && a.cycleId === quarter.id
    );
    
    const totalAllocation = quarterAllocations.reduce((sum, a) => sum + a.percentage, 0);
...
const getCurrentTeamAllocations = (teamId: string, data: PlanningEngineData): any[] => {
  const now = new Date();
  const currentAllocations = data.allocations.filter(a => 
    a.teamId === teamId && 
    data.cycles.some(c => 
      c.id === a.cycleId && 
  ...
  return currentAllocations.map(allocation => {
    ...
    if (allocation.epicId) {
      const epic = data.projects.flatMap(p => p.milestones).find(m => m.id === allocation.epicId);
      if (epic) {
        const project = data.projects.find(p => p.milestones.some(m => m.id === allocation.epicId));
    ...
    return {
      projectId: allocation.epicId ? 
        data.projects.find(p => p.milestones.some(m => m.id === allocation.epicId))?.id || '' : '',
      projectName,
...
const calculateProjectBudgetImpact = (
  ...
  return {
    ...
    costPerTeam: teamRecommendations.map(team => ({
      ...
      currentSize: data.people.filter(p => p.teamId === team.teamId && p.isActive).length,
      totalCost: 0,
...
const assessProjectRisks = (
  ...
  // Skill gap risks
  const criticalSkillGaps = skillRequirements.filter(req => 
    req.importance === 'critical' && 
    !teamRecommendations.some(team => 
...
const calculateFeasibilityScore = (
  ...
  const avgTeamScore = teamRecommendations.reduce((sum, team) => sum + team.matchScore, 0) / teamRecommendations.length;
  const riskPenalty = risks.reduce((sum, risk) => sum + risk.impact, 0) / 100;
  
  return Math.max(0, avgTeamScore - riskPenalty);
};

const calculateProjectDuration = (project: Project, cycles: Cycle[]): number => {
  ...
  const avgIterationLength = iterationCycles.reduce((sum, cycle) => {
    const duration = new Date(cycle.endDate).getTime() - new Date(cycle.startDate).getTime();
    return sum + duration;
...
export const createPlanningScenario = (
  ...
  return {
    ...
    teamChanges: [],
    budgetImpact: {
      totalProjectCost: analyses.reduce((sum, a) => sum + a.budgetRequirement, 0),
      quarterlyBreakdown: [],
      divisionBudgetImpact: [],
      costPerTeam: [],
      runWorkImpact: {
    ...
    feasibilityScore: analyses.reduce((sum, a) => sum + a.feasibilityScore, 0) / analyses.length,
    riskAssessment: [],
