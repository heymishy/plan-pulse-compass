...
import { DivisionBudget, ProjectFeasibilityAnalysis, PlanningScenario } from '@/types/planningTypes';
...
export interface OptimizationRecommendation {
  type: 'budget-reallocation' | 'team-restructure' | 'project-prioritization' | 'capacity-adjustment';
  title: string;
  description: string;
...
export const optimizeBudgetAllocation = (
  ...
  const scenarios = generateOptimizationScenarios(
    currentState, projects, projectAnalyses, config
  );
  
  // Generate recommendations
  const recommendations = generateOptimizationRecommendations(
    currentState, scenarios, config
  );
  
  // Calculate budget impact
  const budgetImpact = calculateBudgetOptimizationImpact(
    currentState, scenarios[0] // Use the best scenario
  );
...
const calculateCurrentState = (
  ...
  return {
    ...
    teamCosts,
    avgUtilization: teamCosts.reduce((sum, tc) => sum + tc.utilization, 0) / teamCosts.length
  };
};
...
const generateOptimizationScenarios = (
  ...
  scenarios.push({
    ...
    description: 'Optimize budget allocation across divisions based on capacity and project priorities',
    projectIds: projectAnalyses.map(pa => pa.projectId),
    teamChanges: [],
    budgetImpact: {
      totalProjectCost: projectAnalyses.reduce((sum, pa) => sum + pa.budgetRequirement, 0) * 0.92,
      quarterlyBreakdown: [],
  ...
  scenarios.push({
    ...
    description: 'Optimize team utilization and capacity allocation for maximum efficiency',
    projectIds: projectAnalyses.map(pa => pa.projectId),
    teamChanges: [
      {
    ...
    budgetImpact: {
      totalProjectCost: projectAnalyses.reduce((sum, pa) => sum + pa.budgetRequirement, 0),
      quarterlyBreakdown: [],
      divisionBudgetImpact: [],
      costPerTeam: [],
      runWorkImpact: {
  ...
  scenarios.push({
    ...
    description: 'Combined budget reallocation and capacity optimization approach',
    projectIds: projectAnalyses.map(pa => pa.projectId),
    teamChanges: [
      {
        type: 'add-person',
    ...
    budgetImpact: {
      totalProjectCost: projectAnalyses.reduce((sum, pa) => sum + pa.budgetRequirement, 0) * 0.95,
      quarterlyBreakdown: [],
      divisionBudgetImpact: [],
      costPerTeam: [],
      runWorkImpact: {
...
const generateOptimizationRecommendations = (
  ...
  if (currentState.budgetUtilization < 85) {
    recommendations.push({
      ...
      details: {
        availableBudget: (100 - currentState.budgetUtilization) * currentState.totalBudget / 100,
  ...
  const underutilizedTeams = currentState.teamCosts.filter((tc: any) => tc.utilization < config.minTeamUtilization);
  if (underutilizedTeams.length > 0) {
    recommendations.push({
      ...
      timeframe: '2-4 weeks',
      details: {
        affectedTeams: underutilizedTeams.map((t: any) => t.teamName),
  ...
  recommendations.push({
    ...
    description: 'Prioritize projects based on ROI and strategic alignment to maximize value delivery.',
    impact: 85,
    effort: 'high',
...
export const simulateOptimizationScenarios = (
  ...
  optimizationOptions.forEach((option, index) => {
    const scenario: PlanningScenario = {
      ...
      feasibilityScore: baselineScenario.feasibilityScore + (Math.random() - 0.5) * 20,
      budgetImpact: {
        ...baselineScenario.budgetImpact,
        totalProjectCost: baselineScenario.budgetImpact.totalProjectCost * (0.9 + Math.random() * 0.2)
      }
    };
