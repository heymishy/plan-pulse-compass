...
export const useCanvasInteractions = ({
  ...
  const handleNodeContextMenu = useCallback(
    (event: React.MouseEvent, node: Node) => {
      ...
      console.log("handleNodeContextMenu - node.id:", node.id);

      if (node.id.startsWith("goal-") || node.id.startsWith("north-star-")) {
        const goalId = node.id.replace("goal-", "").replace("north-star-", "");
        console.log("handleNodeContextMenu - extracted goalId:", goalId);
