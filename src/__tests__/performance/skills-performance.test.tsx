...
const generateTeams = (count: number, skillsPerTeam: number, totalSkills: number): Team[] =>
  Array.from({ length: count }, (_, i) => ({
    id: `team-${i}`,
    name: `Team ${i}`,
    ...
    divisionId: 'dev',
    capacity: 40,
    targetSkills: Array.from({ length: skillsPerTeam }, (_, j) => 
      `skill-${(i * skillsPerTeam + j) % totalSkills}`
    ),
    createdDate: '2024-01-01T00:00:00Z',
    lastModified: '2024-01-01T00:00:00Z'
  }));

const generateSolutions = (count: number, skillsPerSolution: number, totalSkills: number): Solution[] =>
  Array.from({ length: count }, (_, i) => ({
    id: `solution-${i}`,
    name: `Solution ${i}`,
    description: `Performance test solution ${i}`,
    category: 'web-application',
    skills: Array.from({ length: skillsPerSolution }, (_, j) =>
      `skill-${(i * skillsPerSolution + j) % totalSkills}`
    ),
    complexity: 'medium',
    estimatedEffort: 50,
    createdDate: '2024-01-01T00:00:00Z',
...
describe('Skills Performance Tests', () => {
  describe('Skill-Based Planning Performance', () => {
    it('should handle large skill sets efficiently (1000 skills, 100 teams)', () => {
      ...
      const requiredSkills = ['skill-0', 'skill-1', 'skill-2', 'skill-3', 'skill-4'];

      const startTime = performance.now();
      
      // Test compatibility calculation for all teams
      const compatibilityResults = teams.map(team => 
        calculateTeamProjectCompatibility(team, requiredSkills, skills)
      );
      ...
      expect(compatibilityResults.every(result => 
    ...
    it('should efficiently find best teams from large pool (500 teams)', () => {
      ...
      const bestTeams = findBestTeamForProject(
        { requiredSkills, importance: 'high' } as any,
        teams,
        skills
      );
      ...
      expect(bestTeams[0].compatibility.compatibilityScore).toBeGreaterThanOrEqual(
        bestTeams[1].compatibility.compatibilityScore
      );
    });
    ...
    it('should handle complex skill coverage analysis efficiently', () => {
      ...
      expect(coverageAnalysis.coveredSkills).toBeLessThanOrEqual(coverageAnalysis.totalSkills);
  ...
  describe('Component Rendering Performance', () => {
    it('should render SkillsBasedTeamFilter with large datasets quickly', () => {
      ...
      expect(duration).toBeLessThan(200);
      expect(screen.getByTestId('skills-based-team-filter')).toBeInTheDocument();
    });
    ...
    it('should render ProjectSolutionsSkillsSection with many solutions efficiently', () => {
      ...
      const projectSolutions: ProjectSolution[] = solutions.slice(0, 20).map((solution, i) => ({
        id: `ps-${i}`,
      ...
      render(
        <AppProvider value={mockContext}>
          <ProjectSolutionsSkillsSection
            ...
            projectSkills={[]}
            onUpdateProjectSolutions={vi.fn()}
            onUpdateProjectSkills={vi.fn()}
          />
        </AppProvider>
      );
      ...
      expect(screen.getByTestId('project-solutions-skills-section')).toBeInTheDocument();
    });
  });
  ...
  describe('Memory Usage and Cleanup', () => {
    ...
    it('should efficiently update when skill data changes', () => {
      ...
      const updatedSkills = [
        ...initialSkills,
        ...generateSkills(50).map(skill => ({ ...skill, id: `new-${skill.id}` }))
      ];
  ...
  describe('Edge Case Performance', () => {
    it('should handle empty datasets without performance penalty', () => {
      ...
      expect(screen.getByText('Select skills to filter teams')).toBeInTheDocument();
    });
    ...
    it('should handle teams with no skills efficiently', () => {
      ...
      expect(results.every(result => result.compatibilityScore === 0)).toBe(true);
    });
    ...
    it('should handle projects requiring all available skills', () => {
      ...
      const bestTeam = findBestTeamForProject(
        { requiredSkills: allSkillIds, importance: 'critical' } as any,
        teams,
        skills
      );
